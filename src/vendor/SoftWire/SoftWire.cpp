...
#endif
	{
		uNode.digitalWrite(sda, LOW);
		uNode.pinMode(sda, OUTPUT);
	}
}
...
{
	uNode.pinMode(p->getSda(), p->getInputMode());
}
...
#endif
	{
		uNode.digitalWrite(scl, LOW);
		uNode.pinMode(scl, OUTPUT);
	}
}
...
{
	uNode.pinMode(p->getScl(), p->getInputMode());
}
...
{
	return uNode.digitalRead(p->getSda());
}
...
{
	return uNode.digitalRead(p->getScl());
}
