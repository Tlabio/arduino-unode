...
0.8b5: Removed SPI interrupt to make it compatible with ESP8266
0.8b6: Added support for INPUT_PULLUP mode on pinMode
---------------------------------------------------------------------------------------------------------------------
    Copyright (c) 2013-2014, s.u.m.o.t.o.y [sumotoy(at)gmail.com]
    Copyright (c) 2019 Ioannis Charalampidis [ioannis.charalampidis@tlab.gr]
---------------------------------------------------------------------------------------------------------------------
...
public:
  ...
  void        gpioPinMode(uint8_t pin, uint8_t mode);     // set a unique pin as IN(1), OUT (0) or IN_PULLUP(2)
